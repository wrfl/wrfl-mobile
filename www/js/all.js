...
var clean_artist_name_for_echonest, clean_up_now_playing_response, fix_encoded_characters, get_art, get_now_playing, hide_about, pause_stream, play_stream, refresh, show_about, show_notification, toggle_stream, toggle_vinyl, update_ui;

this.echonest_api_key = "RGNDUZEMOHI7QURCL";

this.now_playing_url = "http://wrfl.fm/now.json";

this.stream_url = "http://wrfl-server.ad.uky.edu:9000/stream/1/";

this.refresh_rate_in_seconds = 30;
...
clean_artist_name_for_echonest = (function(_this) {
  return function(name) {
    return name.replace(' ', '+').replace('&', '+');
  };
})(this);

clean_up_now_playing_response = (function(_this) {
...
fix_encoded_characters = (function(_this) {
...
get_art = (function(_this) {
  return function() {
    var name;
    name = clean_artist_name_for_echonest(_this.now_playing.artist);
    return $.get("http://developer.echonest.com/api/v4/artist/images?api_key=" + _this.echonest_api_key + "&name=" + name, function(response) {
      var images, index;
      ...
      if (images.length > 0) {
        ...
        _this.now_playing.art_url = images[index].url;
        return update_ui();
      }
    });
  };
...
get_now_playing = (function(_this) {
  return function() {
    return $.getJSON(_this.now_playing_url, function(response) {
      _this.now_playing = clean_up_now_playing_response(response);
      get_art();
      return update_ui();
    });
  };
...
hide_about = (function(_this) {
  return function() {
    return $('.about').removeClass('visible');
  };
})(this);

pause_stream = (function(_this) {
  return function() {
    if (_this.player) {
...
refresh = (function(_this) {
  return function() {
    get_now_playing();
    return setTimeout(refresh, _this.refresh_rate_in_seconds * 1000);
  };
})(this);

show_about = (function(_this) {
  return function() {
    return $('.about').addClass('visible');
  };
})(this);

show_notification = (function(_this) {
  return function(message, data, id) {
    ...
    return window.plugin.notification.local.onclick = function(id, state, json) {
      return console.log(id, JSON.parse(json).test);
    };
  };
})(this);

toggle_stream = (function(_this) {
  return function() {
    if (_this.player) {
      return pause_stream();
    } else {
...
toggle_vinyl = (function(_this) {
  return function() {
    ...
    if (_this.is_playing) {
      return $('.vinyl').addClass('playing');
    } else {
      return $('.vinyl').removeClass('playing');
    }
  };
})(this);

update_ui = (function(_this) {
  return function() {
    var art_url;
    $('.track').text('"' + _this.now_playing.track + '"');
    $('.artist').text(_this.now_playing.artist);
    $('.album').text(_this.now_playing.album);
    $('.time').text("Played at " + moment(_this.now_playing.played_at * 1000).format("h:mma"));
    $('.dj').text("by " + _this.now_playing.dj);
    art_url = _this.now_playing.art_url || '/assets/default.jpg';
    return $('.art .record').transition({
      opacity: 0
    }, 1000, function() {
      $('.record .image').css({
        'backgroundImage': "url(" + art_url + ")"
      });
      return $('.art .record').delay(1000).transition({
        opacity: 1
      }, 1000);
    });
  };
...
$(function() {
  ...
  $('.about').swipe({
    swipeLeft: hide_about,
    swipeRight: hide_about
  });
